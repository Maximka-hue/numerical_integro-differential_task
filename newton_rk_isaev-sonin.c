...
int main(void) {
    ...
    y_0[1] = alpha[0];
    printf("\nshooting parameters: alpha_1 = %f\n", alpha[0]);

    y = RK(file, true, x_l, x_r, y_0, y_new, tol);
    printf("x1 =  %.10f, x2 =  %.10f \n", y[0], y[1]);
    R0 = fabs(y[0] - y_T[0]); // ошибка по x1(1)
    printf("R0 = %.20f\n", R0);
    free(y_new); free(y_0);
    fclose(file);;
    return 0;
 ...
 */
double f(double x, double* y, double shift_y, int j) {
    double x1 =  y[0] + shift_y, x2 =  y[1] + shift_y;
...
double* RK(FILE *file, bool print, double x_l, double x_r, double* y_0, double* y_new, double tol) {
    ...
    while (x_old < x_r- 1.5*h) {
        ...
        err = fmax( fabs(y_new[0] - w_new[0]), fabs(y_new[1] - w_new[1]))/31;
